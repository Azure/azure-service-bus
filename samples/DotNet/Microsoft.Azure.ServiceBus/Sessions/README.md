...
```Message.SessionId``` property to some application-defined identifier that is unique to 
...
* ```IMessageSessionHandler.OnMessage```/``IMessageSessionAsyncHandler.RegisterMessageHandler``` - this method is
...
* ```IMessageSessionHandler.OnCloseSession```/``IMessageSessionAsyncHandler.OnCloseSessionAsync``` - this 
  ...
  handle any shutdown work it may need to perform. It is not called when the ```OnMessage```/```RegisterMessageHandler``` 

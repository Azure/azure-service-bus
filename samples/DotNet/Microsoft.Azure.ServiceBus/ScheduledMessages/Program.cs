...
namespace MessagingSamples
{
    ...
    public class Program : IBasicQueueSendReceiveSample
    {
        public async Task Run(string namespaceAddress, string queueName, string sendToken, string receiveToken)
        {
            ...
            var sendTask = this.SendMessagesAsync(namespaceAddress, queueName, sendToken, cts.Token);
        ...
        async Task SendMessagesAsync(string namespaceAddress, string queueName, string sendToken, CancellationToken token)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                var message = new BrokeredMessage(new MemoryStream(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(this.data[i]))))
                {
                    ContentType = "application/json",
                    Label = "Scientist",
                    MessageId = i.ToString(),
                    TimeToLive = TimeSpan.FromMinutes(2),
                    ScheduledEnqueueTimeUtc = now + TimeSpan.FromSeconds(30+i)
                };
                ...
                lock (Console.Out)
                {
                    ...
                    Console.WriteLine("Message sent: Id = {0}, scheduled for UTC {1}", message.MessageId, message.ScheduledEnqueueTimeUtc);
                    Console.ResetColor();
                }
            }

...
```C#
 this.sendClient = new QueueClient(connectionString, queueName);
```
...
```C#
  this.receiveClient = new QueueClient(connectionString, queueName, ReceiveMode.PeekLock);
```
...
the vast majority of applications will *and should* instead use the ```OnMessage``` or ```RegisterMessageHandler``` API. 

> **Unless you have good reason to create your own receive loop, you should use the ```OnMessage```/```RegisterMessageHandler``` API**
...
The receive loop will be started as soon as ```RegisterMessageHandler``` is invoked and will continue in the background when the method 
has returned. ```OnMessage```/```RegisterMessageHandler``` can only be called once on any receiver. The receive loop stops when the ```QueueClient``` is 
...
previously discussed. The receive client gets created and is then initialized with the ``RegisterMessageHandler`` callback, which 
...
As the user presses any key, the ```receiveClient``` is closed, which stops the ```RegisterMessageHandler``` receive loop. If 

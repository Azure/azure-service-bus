...
#Optional – you can obtain the alias details using this cmdlet, here you can also check if the provisioning is done. Provisioning state should change from Accepted to Succeeded.
...
# Deleting the alias would then break the pairing and at this point primary and secondary namespaces are no more in sync 
 Remove-AzureRmServiceBusGeoDRConfiguration -ResourceGroupName $resourcegroup -Name $aliasname
